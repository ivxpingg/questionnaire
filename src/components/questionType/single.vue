<template lang="html">
    <li :class="valdateSuccess ? '' : 'waning-bg'">
        <div class="form-horizontal s-form-inline" v-show="bEdit_com">
           <div class="form-group form-inline">
               <label class="control-label">{{num + 1}}、单选题</label>
               <span class="checkbox checkbox-primary">
                   <input :id="'chk_single_bt' + num" :checked="questions.isRequired == '0' ? false : true" @change="bRequiredChange" type="checkbox">
                   <label :for="'chk_single_bt' + num">必填</label>
               </span>
               <span class="checkbox checkbox-primary">
                   <input :id="'chk_single_tj' + num" :checked="questions.isPublic == '0' ? false : true" @change="bPublicChange" type="checkbox">
                   <label :for="'chk_single_tj' + num">公开统计结果</label>
               </span>
            </div>
...
export default {
    ...
    data: function () {
        return {
            // 是否展示添加 其他 项
            bBtnAnswerOther: true,
            valdateSuccess: true,   // 判断是否通过表单验证
            bEdit: true             // 是否是编辑状态
