    ...
    <li>
        <div class="form-horizontal s-form-inline" v-show="bEdit_com">
           <div class="form-group form-inline">
               <label class="control-label">{{num + 1}}、位置拾取题</label>
               <span class="checkbox checkbox-primary">
                   <input :id="'chk_position_bt' + num" :checked="questions.isRequired === '0' ? false : true" @change="bRequiredChange" type="checkbox">
                   <label :for="'chk_position_bt' + num">必填</label>
               </span>
               <span class="checkbox checkbox-primary">
                   <input :id="'chk_position_tj' + num" :checked="questions.isPublic === '0' ? false : true" @change="bPublicChange" type="checkbox">
                   <label :for="'chk_position_tj' + num">公开统计结果</label>
               </span>
            </div>
        ...
        <div class="" v-show="!bEdit_com"  @click="switchEdit">
            ...
            <div class="item-position-answer" v-for="(ans, n) in questions.answers">
                <i class="p-icon glyphicon glyphicon-map-marker"></i>
                <label class="p-label" :for="'item_position_' + num + '_' + n">{{ans.value}}</label>
                <input class="p-form-control form-control" disabled :id="'item_position_' + num + '_' + n" type="text" name="">
                <!-- <span class="radio radio-primary">
                    <input type="radio" :name="'item_single_name' + num + '_' + n" disabled :id="'item_single_' + num + '_' + n" value="">
                    <label :for="'item_single_' + num + '_' + n">{{String.fromCharCode(65 + n) + ans.value}}</label>
                </span> -->
            </div>
        </div>
